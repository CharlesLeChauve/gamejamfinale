        ...
        while (data->map[y][x])
        {
            ...
            else if (data->map[y][x] == 'E')
            {
                if (ft_cookiz(data->map) != 0)
                    mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->exit_close.sprite.mlx_img, ft_coordonnees(x, data), ft_coordonnees(y, data));
                else
                    mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->exit_open.sprite.mlx_img, ft_coordonnees(x, data), ft_coordonnees(y, data));
